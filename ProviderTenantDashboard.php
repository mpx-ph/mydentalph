...
<body class="bg-background font-body text-on-background mesh-bg min-h-screen flex text-[15px] leading-normal">
<aside class="fixed left-0 top-0 h-full w-56 z-40 sidebar-bg flex flex-col py-5 border-r border-slate-200/60" data-purpose="navigation-sidebar">
<div class="px-4 mb-5">
<img src="MyDental%20Logo.svg" alt="MyDental" width="144" height="36" loading="eager" decoding="async" class="h-7 w-auto max-w-[132px] object-contain object-left"/>
<p class="text-primary font-bold text-[9px] tracking-[0.18em] uppercase mt-2 opacity-80">Provider Console</p>
</div>
<nav class="flex-1 space-y-0.5 overflow-y-auto no-scrollbar px-0">
<div class="relative px-2">
<a class="flex items-center gap-2.5 px-3 py-2.5 bg-primary/10 text-primary rounded-lg transition-all duration-200 active-glow" data-purpose="nav-item" href="#">
<span class="material-symbols-outlined text-lg" style="font-variation-settings: 'FILL' 1;">dashboard</span>
<span class="font-headline text-xs font-bold tracking-tight">Dashboard</span>
</a>
<div class="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 bg-primary rounded-r-full"></div>
</div>
<div class="px-2">
<a class="flex items-center gap-2.5 px-3 py-2.5 text-slate-600 hover:text-slate-900 transition-colors duration-200 hover:bg-slate-50 rounded-lg text-xs font-headline font-medium" data-purpose="nav-item" href="#">
...
<div class="px-2 pb-1">
<a class="flex items-center gap-2.5 px-3 py-2.5 text-rose-600 hover:text-rose-700 transition-colors duration-200 hover:bg-rose-50 rounded-lg text-xs font-headline font-bold" href="ProviderLogout.php">
...
<div class="w-8 h-8 rounded-full bg-primary/20 flex items-center justify-center text-primary text-[10px] font-bold shrink-0"><?php echo htmlspecialchars($avatar_initials); ?></div>
<div class="min-w-0 flex-1">
<p class="text-slate-900 text-[11px] font-bold truncate"><?php echo htmlspecialchars($plan_name); ?></p>
<p class="text-slate-500 text-[9px] truncate"><?php echo htmlspecialchars($renewal_sidebar); ?></p>
</div>
</div>
<p class="text-[9px] text-slate-500 truncate mb-2"><?php echo htmlspecialchars($clinic_name); ?></p>
<a class="block w-full text-center py-2 bg-white border border-slate-200 hover:border-primary/30 text-slate-700 text-[10px] font-bold rounded-lg transition-all shadow-sm" href="<?php echo $has_visible_website && $tenant_base_url ? htmlspecialchars($tenant_base_url . '/', ENT_QUOTES, 'UTF-8') : '#'; ?>" <?php if ($has_visible_website && $tenant_base_url): ?>target="_blank" rel="noopener noreferrer"<?php endif; ?>>Visit website</a>
</div>
</div>
</aside>
<main class="flex-1 flex flex-col min-w-0 ml-56">
<header class="flex justify-between items-center w-full px-5 sticky top-0 z-40 bg-white/80 backdrop-blur-xl h-14 border-b border-slate-200/50" data-purpose="top-header">
<div class="flex flex-wrap items-center gap-3 sm:gap-5">
<div class="flex items-center gap-1.5">
<span class="w-1.5 h-1.5 rounded-full <?php echo $has_visible_website ? 'bg-green-500 animate-pulse' : 'bg-amber-500'; ?>"></span>
<span class="font-headline text-[9px] font-black uppercase tracking-[0.18em] text-primary">Website: <?php echo $has_visible_website ? 'Online' : 'Not published'; ?></span>
</div>
<div class="hidden sm:block h-3 w-px bg-slate-200"></div>
<span class="font-headline text-[9px] font-black uppercase tracking-[0.18em] text-slate-400 truncate max-w-[160px] sm:max-w-none">Plan: <?php echo htmlspecialchars($plan_name); ?></span>
</div>
<div class="flex items-center gap-3">
<div class="hidden sm:flex items-center gap-4 text-on-surface-variant/60">
<button type="button" class="material-symbols-outlined text-xl hover:text-primary transition-colors bg-transparent border-0 cursor-pointer p-0" aria-label="Notifications">notifications</button>
<button type="button" class="material-symbols-outlined text-xl hover:text-primary transition-colors bg-transparent border-0 cursor-pointer p-0" aria-label="Help">help_outline</button>
</div>
<div class="h-8 w-8 rounded-full overflow-hidden border border-primary/20 bg-primary/10 flex items-center justify-center text-primary text-[10px] font-black shrink-0" aria-hidden="true"><?php echo htmlspecialchars($avatar_initials); ?></div>
</div>
</header>
<div class="p-5 max-w-6xl mx-auto w-full space-y-6 flex-1">
<section class="flex flex-col gap-2">
<div class="text-primary font-bold text-[10px] uppercase flex items-center gap-3 tracking-[0.2em]">
<span class="w-8 h-px bg-primary"></span> Tenant dashboard
            </div>
<h2 class="font-headline text-2xl sm:text-3xl font-extrabold tracking-tight text-on-background">
                Clinic <span class="font-editorial italic font-normal text-primary editorial-word">Overview</span>
</h2>
<p class="font-body text-sm text-on-surface-variant max-w-2xl leading-relaxed">Welcome back, <?php echo htmlspecialchars($welcome_name); ?>. Open clinic management, review your plan and website, and keep account details up to date.</p>
</section>
<section class="glass-card rounded-xl p-5 border border-slate-200/80 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4" data-purpose="primary-action-card">
<div class="space-y-1">
<h2 class="font-headline text-lg font-extrabold tracking-tight text-on-background">Access clinic management</h2>
<p class="text-on-surface-variant text-xs max-w-md">Manage patients, appointments, and records from your clinic admin dashboard.</p>
</div>
<a
  class="inline-flex items-center justify-center shrink-0 bg-primary text-white px-5 py-2.5 rounded-xl font-headline font-bold text-sm hover:bg-blue-600 transition-all shadow-md shadow-primary/20 text-center"
...
<div class="space-y-5">
<div class="grid grid-cols-1 md:grid-cols-3 gap-4" data-purpose="overview-stats">
<div class="glass-card p-5 rounded-xl flex flex-col justify-between hover:shadow-lg hover:shadow-primary/5 transition-all duration-300">
<div class="flex justify-between items-start gap-2">
<div class="min-w-0">
<p class="text-on-surface-variant font-bold text-[9px] uppercase tracking-widest mb-2">Current Plan</p>
<h3 class="font-headline text-lg font-extrabold text-primary leading-snug break-words"><?php echo htmlspecialchars($plan_name); ?></h3>
</div>
<?php if ($is_subscription_active): ?>
<span class="shrink-0 bg-primary/10 text-primary px-2 py-1 rounded-full text-[9px] font-black uppercase tracking-wider">Active</span>
<?php elseif ($subscription_state === 'expired'): ?>
<span class="shrink-0 bg-amber-100 text-amber-800 px-2 py-1 rounded-full text-[9px] font-black uppercase tracking-wider">Expired</span>
<?php elseif ($subscription_state === 'inactive'): ?>
<span class="shrink-0 bg-slate-200 text-slate-800 px-2 py-1 rounded-full text-[9px] font-black uppercase tracking-wider">Inactive</span>
<?php else: ?>
<span class="shrink-0 bg-slate-100 text-slate-600 px-2 py-1 rounded-full text-[9px] font-black uppercase tracking-wider">None</span>
<?php endif; ?>
</div>
<div class="space-y-3 mt-4">
<?php if ($plan_billing_cycle_label !== ''): ?>
<p class="text-on-surface-variant/80 text-[11px] font-semibold"><?php echo htmlspecialchars($plan_billing_cycle_label); ?></p>
<?php endif; ?>
<?php if ($has_subscription_row && $period_start_ts !== false && $renewal_ts !== false): ?>
<div class="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
<div class="bg-primary h-full rounded-full shadow-[0_0_8px_rgba(43,139,235,0.35)] transition-all" style="width: <?php echo (int) $plan_period_util_pct; ?>%;"></div>
</div>
<div class="flex flex-col sm:flex-row sm:justify-between gap-1 text-[10px] font-black uppercase tracking-wider text-on-surface-variant/70">
...
<p class="text-on-surface-variant text-xs">Current period: <?php echo htmlspecialchars($period_start_display); ?> — <?php echo htmlspecialchars($renewal_date); ?></p>
<?php else: ?>
<p class="text-on-surface-variant/80 text-xs">No subscription record in the database yet.</p>
<?php endif; ?>
</div>
</div>
<div class="glass-card p-5 rounded-xl flex flex-col justify-between hover:shadow-lg hover:shadow-primary/5 transition-all duration-300">
<div>
<p class="text-on-surface-variant font-bold text-[9px] uppercase tracking-widest mb-2">Domain &amp; Hosting</p>
<?php if ($has_visible_website): ?>
<h3 class="font-headline text-base font-extrabold tracking-tight break-all"><?php echo htmlspecialchars($domain_display); ?></h3>
<a class="inline-flex items-center gap-1.5 text-primary text-[10px] font-black uppercase tracking-wider mt-3 hover:gap-2 transition-all" href="<?php echo htmlspecialchars($tenant_base_url . '/', ENT_QUOTES, 'UTF-8'); ?>" target="_blank" rel="noopener noreferrer">
                                Visit Website
                                <span class="material-symbols-outlined text-sm">arrow_forward</span>
</a>
<?php else: ?>
<h3 class="font-headline text-base font-extrabold tracking-tight text-on-background"><?php echo htmlspecialchars($domain_display); ?></h3>
<?php endif; ?>
<p class="text-[11px] font-bold text-on-surface-variant/80 mt-3"><?php echo htmlspecialchars($hosting_status_label); ?></p>
</div>
<div class="flex items-center gap-2 mt-4 pt-4 border-t border-slate-100">
<span class="material-symbols-outlined <?php echo $has_visible_website ? 'text-green-500' : 'text-amber-500'; ?> text-base">verified_user</span>
<span class="text-[10px] font-bold text-on-surface-variant/70 uppercase tracking-wider"><?php echo $has_visible_website ? 'Website published' : 'Website not yet published'; ?></span>
</div>
</div>
<div class="glass-card p-5 rounded-xl hover:shadow-lg hover:shadow-primary/5 transition-all duration-300 flex flex-col">
<h4 class="font-headline font-extrabold text-sm mb-4">Website <span class="text-primary italic font-editorial">Control</span></h4>
<div class="flex flex-wrap gap-2 mb-3">
<button type="button" class="bg-primary/10 text-primary hover:bg-primary hover:text-white px-3 py-2 rounded-lg text-[10px] font-black uppercase tracking-wide transition-colors">Publish</button>
<button type="button" class="bg-slate-100 text-slate-600 hover:bg-slate-200 px-3 py-2 rounded-lg text-[10px] font-black uppercase tracking-wide transition-colors">Unpublish</button>
</div>
<a
  class="inline-flex items-center gap-1.5 text-primary text-[10px] font-black uppercase tracking-wider mt-auto hover:gap-2 transition-all"
...
<div class="glass-card rounded-xl p-5 border border-slate-200/80">
<h4 class="font-headline font-extrabold text-sm mb-4">Infrastructure <span class="text-primary italic font-editorial">Status</span></h4>
<div class="space-y-3">
<div class="flex items-center justify-between gap-3">
<span class="text-[11px] font-black uppercase tracking-wider text-on-surface-variant/70">Database</span>
<span class="text-[9px] font-black text-green-500 flex items-center gap-1.5 uppercase tracking-[0.15em] shrink-0">
<span class="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></span>
                                    Connected
                                </span>
</div>
<div class="flex items-center justify-between gap-3">
<span class="text-[11px] font-black uppercase tracking-wider text-on-surface-variant/70">Clinic portal</span>
<span class="text-[9px] font-black <?php echo $has_visible_website ? 'text-green-500' : 'text-amber-600'; ?> flex items-center gap-1.5 uppercase tracking-[0.15em] shrink-0">
<span class="w-1.5 h-1.5 rounded-full <?php echo $has_visible_website ? 'bg-green-500 animate-pulse' : 'bg-amber-500'; ?>"></span>
                                    <?php echo $has_visible_website ? 'Ready' : 'Unavailable'; ?>
                                </span>
</div>
<div class="flex items-center justify-between gap-3">
<span class="text-[11px] font-black uppercase tracking-wider text-on-surface-variant/70">Subscription</span>
<span class="text-[9px] font-black <?php echo $is_subscription_active ? 'text-green-500' : 'text-slate-500'; ?> flex items-center gap-1.5 uppercase tracking-[0.15em] shrink-0">
<span class="w-1.5 h-1.5 rounded-full <?php echo $is_subscription_active ? 'bg-green-500' : 'bg-slate-400'; ?>"></span>
                                    <?php echo $is_subscription_active ? 'Active' : ucfirst($subscription_state); ?>
                                </span>
</div>
</div>
</div>
<div class="glass-card p-5 sm:p-6 rounded-xl border border-slate-200/80 max-w-3xl">
<?php if ($settings_saved): ?>
<div class="mb-4 p-3 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-lg text-xs font-medium">Settings saved successfully.</div>
<?php endif; ?>
<?php if ($show_activated_banner): ?>
<div class="mb-4 p-3 bg-surface-container-low border border-primary/20 text-primary rounded-lg text-xs font-headline font-bold">Subscription activated. Your clinic website is now live and ready to manage.</div>
<?php endif; ?>
<?php if ($settings_error): ?>
<div class="mb-4 p-3 bg-red-50 border border-red-200 text-red-700 rounded-lg text-xs"><?php echo htmlspecialchars($settings_error); ?></div>
<?php endif; ?>
<div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-5">
<div>
<h3 class="text-lg font-headline font-extrabold text-on-background">Account Settings</h3>
<p class="text-on-surface-variant text-xs mt-0.5">Clinic details (tbl_tenants) and your account (tbl_users).</p>
</div>
<button class="px-4 py-2 bg-primary text-white rounded-lg font-headline text-sm font-bold hover:bg-blue-600 transition-all shadow-sm shadow-primary/20 shrink-0" form="settings-form" type="submit">Save Changes</button>
</div>
<form class="space-y-5" method="post" action="" data-purpose="account-form" id="settings-form">
<?php if ($is_owner): ?>
<div class="border-b border-slate-200 pb-5">
<h4 class="text-[10px] font-black text-on-surface-variant uppercase tracking-[0.15em] mb-3">Clinic details</h4>
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
<div class="md:col-span-2">
<label class="text-xs font-semibold text-slate-700 font-headline" for="clinic_name">Clinic Name</label>
<input class="mt-1 w-full px-3 py-2 text-sm rounded-lg border-slate-200 focus:border-primary focus:ring-primary transition-all" id="clinic_name" name="clinic_name" placeholder="Clinic name" type="text" value="<?php echo htmlspecialchars($tenant['clinic_name'] ?? ''); ?>"/>
</div>
<div>
<label class="text-xs font-semibold text-slate-700 font-headline" for="clinic_email">Clinic Email</label>
<input class="mt-1 w-full px-3 py-2 text-sm rounded-lg border-slate-200 focus:border-primary focus:ring-primary transition-all" id="clinic_email" name="clinic_email" placeholder="Clinic email" type="email" value="<?php echo htmlspecialchars($tenant['contact_email'] ?? ''); ?>"/>
</div>
<div>
<label class="text-xs font-semibold text-slate-700 font-headline" for="clinic_phone">Clinic Phone</label>
<input class="mt-1 w-full px-3 py-2 text-sm rounded-lg border-slate-200 focus:border-primary focus:ring-primary transition-all" id="clinic_phone" name="clinic_phone" placeholder="Clinic phone" type="tel" value="<?php echo htmlspecialchars($tenant['contact_phone'] ?? ''); ?>"/>
</div>
<div class="md:col-span-2">
<label class="text-xs font-semibold text-slate-700 font-headline" for="clinic_address">Clinic Address</label>
<textarea class="mt-1 w-full px-3 py-2 text-sm rounded-lg border-slate-200 focus:border-primary focus:ring-primary transition-all" id="clinic_address" name="clinic_address" placeholder="Address" rows="2"><?php echo htmlspecialchars($tenant['clinic_address'] ?? ''); ?></textarea>
</div>
</div>
</div>
<?php endif; ?>
<div>
<h4 class="text-[10px] font-black text-on-surface-variant uppercase tracking-[0.15em] mb-3">Your account</h4>
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
<div>
<label class="text-xs font-semibold text-slate-700 font-headline" for="full-name">Full Name</label>
<input class="mt-1 w-full px-3 py-2 text-sm rounded-lg border-slate-200 focus:border-primary focus:ring-primary transition-all" id="full-name" name="full_name" placeholder="Full name" type="text" value="<?php echo htmlspecialchars($current_user['full_name'] ?? ''); ?>"/>
</div>
<div>
<label class="text-xs font-semibold text-slate-700 font-headline" for="email">Email Address</label>
<input class="mt-1 w-full px-3 py-2 text-sm rounded-lg border-slate-200 focus:border-primary focus:ring-primary transition-all" id="email" name="email" placeholder="Email" type="email" value="<?php echo htmlspecialchars($current_user['email'] ?? ''); ?>"/>
</div>
<div>
<label class="text-xs font-semibold text-slate-700 font-headline" for="contact">Contact Number</label>
<input class="mt-1 w-full px-3 py-2 text-sm rounded-lg border-slate-200 focus:border-primary focus:ring-primary transition-all" id="contact" name="phone" placeholder="Phone" type="tel" value="<?php echo htmlspecialchars($current_user['phone'] ?? ''); ?>"/>
</div>
</div>
</div>
</form>
