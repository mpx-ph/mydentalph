-- ============================================================================
-- MyDentalPH — Workbench ERD schema (MySQL)
-- Paste into MySQL Workbench and execute on an empty schema.
--
-- Differences vs schema.sql:
--   * tbl_platform: ties Figure 1 tables (tenants, plans, superadmin) together.
--   * Extra FKs on tables that intentionally omitted them (hosting compat).
--   * tbl_tenant_public_services.tenant_id aligned to VARCHAR(20) for FK.
--   * tbl_provider_pending_signups.subscription_plan_id → tbl_subscription_plans.
-- Production: review before applying; add platform columns only if you want them.
-- ============================================================================

SET FOREIGN_KEY_CHECKS = 0;

-- ============================================
-- PLATFORM (single SaaS deployment; ERD hub for Figure 1)
-- ============================================
CREATE TABLE IF NOT EXISTS tbl_platform (
    platform_id TINYINT NOT NULL PRIMARY KEY DEFAULT 1,
    label VARCHAR(100) NOT NULL DEFAULT 'MyDental'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

INSERT IGNORE INTO tbl_platform (platform_id, label) VALUES (1, 'MyDental');
...
CREATE TABLE IF NOT EXISTS tbl_tenants (
    ...
    platform_id TINYINT NOT NULL DEFAULT 1,
    ...
    CONSTRAINT fk_tenants_platform
        FOREIGN KEY (platform_id) REFERENCES tbl_platform(platform_id)
    -- fk_tenants_owner_user added after tbl_users (see end of file)
);
...
CREATE TABLE IF NOT EXISTS tbl_subscription_plans (
    ...
    CONSTRAINT fk_subscription_plans_platform
        FOREIGN KEY (platform_id) REFERENCES tbl_platform(platform_id)
);
...
CREATE TABLE IF NOT EXISTS tbl_payments (
    ...
    payment_type ENUM('downpayment', 'fullpayment', 'balancepayment') NOT NULL DEFAULT 'fullpayment',
    created_by VARCHAR(20),
    PRIMARY KEY (id),
...
CREATE TABLE IF NOT EXISTS tbl_wallet_accounts (
    ...
    CONSTRAINT fk_wallet_accounts_tenant
        FOREIGN KEY (tenant_id) REFERENCES tbl_tenants(tenant_id) ON DELETE CASCADE,
    CONSTRAINT fk_wallet_accounts_patient
...
CREATE TABLE IF NOT EXISTS tbl_wallet_transactions (
    ...
    CONSTRAINT fk_wallet_transactions_tenant
        FOREIGN KEY (tenant_id) REFERENCES tbl_tenants(tenant_id) ON DELETE CASCADE,
    CONSTRAINT fk_wallet_transactions_wallet
        FOREIGN KEY (tenant_id, wallet_id) REFERENCES tbl_wallet_accounts(tenant_id, wallet_id) ON DELETE CASCADE,
    CONSTRAINT fk_wallet_transactions_source_payment
        FOREIGN KEY (tenant_id, source_payment_id) REFERENCES tbl_payments(tenant_id, payment_id) ON DELETE SET NULL,
...
-- ============================================
-- DISCOUNT PROGRAMS & VERIFICATIONS (staff portal)
-- ============================================
-- No FK constraints here (compat with shared hosts); same pattern as tbl_payment_settings.
