...
try {
    ...
    foreach ($rows as $row) {
        $tx = $row;
        $tx['service'] = $row['display_service'] ?: 'Consultation';
        $tx['date']   = $row['appointment_date'];
        $tx['time']   = $row['appointment_time'];
        $tx['dentist'] = 'Dr. ' . trim($row['dentist_name'] ?? '');
        $tx['apt_status'] = strtoupper($row['status'] ?? 'PENDING');
        $tx['total_cost'] = number_format((float)($row['total_treatment_cost'] ?? 0), 2);
        $tx['payment_amount'] = number_format((float)($row['payment_amount'] ?? 0), 2);
        $tx['payment_method'] = strtoupper($row['payment_method'] ?? 'N/A');
        $tx['payment_ref']   = $row['reference_number'] ?: 'N/A';
        $tx['payment_status'] = strtoupper($row['payment_status'] ?? 'UNPAID');
        $transactions[] = $tx;
    }
