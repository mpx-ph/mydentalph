...
<!-- Table Content -->
<div class="md:hidden px-4 sm:px-6 pb-4">
<?php if ($dbError !== null): ?>
<div class="rounded-2xl border border-error/20 bg-error/10 px-4 py-5 text-sm text-error font-medium">
Unable to load tenants.
</div>
<?php elseif (empty($tenants)): ?>
<div class="rounded-2xl border border-outline-variant/30 bg-white/70 px-4 py-5 text-sm text-on-surface-variant font-medium">
...
<?php foreach ($tenants as $row):
    ...
    ?>
<article class="rounded-2xl bg-white/80 border border-white p-4 shadow-sm space-y-3">
<div class="flex items-start justify-between gap-3">
<div class="flex items-center gap-3 min-w-0">
<?php if ($photo !== ''): ?>
<img alt="" class="w-11 h-11 rounded-full object-cover border-2 border-white shadow-sm shrink-0" src="<?php echo htmlspecialchars($photo, ENT_QUOTES, 'UTF-8'); ?>"/>
<?php else: ?>
<div class="w-11 h-11 rounded-full bg-blue-100 flex items-center justify-center text-primary text-xs font-bold border-2 border-white shadow-sm shrink-0"><?php echo htmlspecialchars(tenant_tm_initials($displayName === '—' ? (string) $row['clinic_name'] : $displayName), ENT_QUOTES, 'UTF-8'); ?></div>
<?php endif; ?>
<div class="min-w-0">
<p class="text-sm font-bold text-on-surface truncate"><?php echo htmlspecialchars($displayName, ENT_QUOTES, 'UTF-8'); ?></p>
...
<div class="rounded-xl bg-surface-container-low/60 px-3 py-2">
<p class="text-on-surface-variant uppercase tracking-wide font-bold text-[10px]">Clinic</p>
<button
    type="button"
    class="js-open-tenant-details text-left mt-1 text-sm font-semibold text-primary hover:underline break-words"
...
<div class="rounded-xl bg-surface-container-low/60 px-3 py-2">
<p class="text-on-surface-variant uppercase tracking-wide font-bold text-[10px]">Subscription</p>
<p class="mt-1 text-sm font-semibold text-on-surface"><?php echo $planName !== '' ? htmlspecialchars($planName, ENT_QUOTES, 'UTF-8') : '—'; ?></p>
</div>
</div>
<div class="flex items-center justify-between gap-3">
<p class="text-[11px] text-on-surface-variant font-medium">Last activity: <?php echo tenant_tm_last_activity($row['owner_last_login'] ?? null); ?></p>
<button
    type="button"
    class="js-open-tenant-details inline-flex items-center gap-1 text-primary text-xs font-bold hover:underline"
    data-tenant-id="<?php echo htmlspecialchars((string) $row['tenant_id'], ENT_QUOTES, 'UTF-8'); ?>"
>
...
<form method="post" action="<?php echo htmlspecialchars(tenant_tm_url($filterBase, ['page' => $page, 'wf_page' => $workforcePage]), ENT_QUOTES, 'UTF-8'); ?>" class="min-w-0">
<input type="hidden" name="tenant_id" value="<?php echo htmlspecialchars((string) $row['tenant_id'], ENT_QUOTES, 'UTF-8'); ?>"/>
<button type="submit" name="tenant_action" value="publish" <?php echo $canPublish ? '' : 'disabled'; ?> class="w-full inline-flex items-center justify-center gap-1 px-2 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wide border border-outline-variant/30 <?php echo $canPublish ? 'text-primary bg-white hover:bg-primary/10' : 'opacity-40 cursor-not-allowed bg-white'; ?>"><span class="material-symbols-outlined text-sm">publish</span> Publish</button>
</form>
<form method="post" action="<?php echo htmlspecialchars(tenant_tm_url($filterBase, ['page' => $page, 'wf_page' => $workforcePage]), ENT_QUOTES, 'UTF-8'); ?>" class="min-w-0">
<input type="hidden" name="tenant_id" value="<?php echo htmlspecialchars((string) $row['tenant_id'], ENT_QUOTES, 'UTF-8'); ?>"/>
<button type="submit" name="tenant_action" value="unpublish" <?php echo $canUnpublish ? '' : 'disabled'; ?> class="w-full inline-flex items-center justify-center gap-1 px-2 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wide border border-outline-variant/30 <?php echo $canUnpublish ? 'text-on-surface-variant bg-white hover:bg-white/80' : 'opacity-40 cursor-not-allowed bg-white'; ?>"><span class="material-symbols-outlined text-sm">unpublished</span> Unpublish</button>
</form>
<form method="post" action="<?php echo htmlspecialchars(tenant_tm_url($filterBase, ['page' => $page, 'wf_page' => $workforcePage]), ENT_QUOTES, 'UTF-8'); ?>" class="min-w-0">
<input type="hidden" name="tenant_id" value="<?php echo htmlspecialchars((string) $row['tenant_id'], ENT_QUOTES, 'UTF-8'); ?>"/>
<button type="submit" name="tenant_action" value="suspend" <?php echo $canSuspend ? '' : 'disabled'; ?> class="w-full inline-flex items-center justify-center gap-1 px-2 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wide border border-outline-variant/30 <?php echo $canSuspend ? 'text-error bg-white hover:bg-error/10' : 'opacity-40 cursor-not-allowed bg-white'; ?>"><span class="material-symbols-outlined text-sm">block</span> Suspend</button>
</form>
</div>
</article>
<?php endforeach; ?>
